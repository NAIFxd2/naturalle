...
export default function AprovarSolicitacoes() {
    ...
    const pendentes = solicitacoes.filter(s => s.status === 'pendente_vistoria').sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    const historico = solicitacoes.filter(s => s.vistoriaApproval != null).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    const approve = () => {
        updateSolicitacao(selectedSol.id, {
            status: 'aprovado_vistoria',
            vistoriaApproval: 'aprovado',
            vistoriaDate: new Date().toISOString(),
            vistoriaObs: obs,
        });
        setModalOpen(false);
    };
    ...
    const reject = () => {
        updateSolicitacao(selectedSol.id, {
            status: 'rejeitado_vistoria',
            vistoriaApproval: 'rejeitado',
            vistoriaDate: new Date().toISOString(),
            vistoriaObs: obs,
        });
        setModalOpen(false);
    ...
    return (
        <div>
            <div className="page-header">
                <div>
                    <h1>Aprovar Solicitações</h1>
                    <p>Analise e aprove as solicitações de materiais</p>
                </div>
            </div>
            ...
            <div className="card">
                {currentList.length === 0 ? (
                    <div className="empty-state">
                        ...
                        <h3>{tab === 'pendentes' ? 'Nenhuma solicitação pendente' : 'Nenhum histórico'}</h3>
                        <p>{tab === 'pendentes' ? 'Todas as solicitações foram analisadas.' : 'O histórico aparecerá aqui.'}</p>
                    </div>
                ) : (
                            ...
                            <tbody>
                                {currentList.map(s => (
                                    <tr key={s.id}>
                                        ...
                                        {tab === 'historico' && (
                                            <td>
                                                <span className={`badge ${s.vistoriaApproval === 'aprovado' ? 'badge-success' : 'badge-danger'}`}>
                                                    {s.vistoriaApproval === 'aprovado' ? 'Aprovado' : 'Rejeitado'}
                                                </span>
                                            </td>
                                        )}
                                        <td>
            ...
            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Análise de Solicitação" large
                footer={selectedSol?.status === 'pendente_vistoria' ? (
                    <>
                        <button className="btn btn-danger" onClick={reject}><X size={14} /> Rejeitar</button>
                        <button className="btn btn-success" onClick={approve}><Check size={14} /> Aprovar</button>
                    </>
                ) : null}
            >
                {selectedSol && (
                    <div>
                        <div className="detail-card">
                            ...
                            <div className="detail-row"><span className="detail-label">Quantidade</span><span className="detail-value">{selectedSol.quantidade}</span></div>
                        ...
                        {selectedSol.status === 'pendente_vistoria' && (
                        ...
                        {selectedSol.vistoriaObs && selectedSol.status !== 'pendente_vistoria' && (
